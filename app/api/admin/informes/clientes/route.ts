...
export async function GET(request: NextRequest) {
  ...
  let historicoQuery = supabase.from("citas").select("cliente_id, inicio").eq("estado", "completada");
  ...
  const porCliente = new Map<string, { count: number; primera: string; ultima: string }>();
  for (const c of historico) {
    const actual = porCliente.get(c.cliente_id);
    if (!actual) {
      porCliente.set(c.cliente_id, { count: 1, primera: c.inicio, ultima: c.inicio });
    } else {
      actual.count++;
  ...
  const clientesRecurrentesHistorico = Array.from(porCliente.values()).filter((c) => c.count > 1).length;
  const tasaRecurrenciaHistoricaPct =
    totalClientesConVisita > 0 ? Math.round((clientesRecurrentesHistorico / totalClientesConVisita) * 1000) / 10 : 0;

  const hoyLocal = toZonedTime(new Date(), TZ);
  let clientesEnRiesgo = 0;
  for (const info of porCliente.values()) {
    const dias = differenceInCalendarDays(hoyLocal, toZonedTime(new Date(info.ultima), TZ));
    if (dias >= RIESGO_DESDE_DIAS && dias <= RIESGO_HASTA_DIAS) clientesEnRiesgo++;
  }
  ...
  let rangoQuery = supabase
    .from("citas")
    .select("id, cliente_id, inicio, cliente:clientes(nombre), servicio:servicios(precio_centimos)")
  ...
  const { data: extras } = await supabase
    ...
    .in("cita_id", idsRango.length ? idsRango : ["00000000-0000-0000-0000-000000000000"]);
  const extrasPorCita = new Map<string, number>();
  for (const e of extras ?? []) {
    extrasPorCita.set(e.cita_id, (extrasPorCita.get(e.cita_id) ?? 0) + e.precio_centimos);
  }

  const ingresosPorCliente = new Map<string, { nombre: string; ingresosCentimos: number; visitas: number }>();
  for (const c of citasRango) {
    ...
    const actual = ingresosPorCliente.get(c.cliente_id) ?? { nombre: cliente?.nombre ?? "Cliente", ingresosCentimos: 0, visitas: 0 };
    actual.ingresosCentimos += ingresoCita;
    actual.visitas += 1;
    ingresosPorCliente.set(c.cliente_id, actual);
  }
  const ingresosTotalRangoCentimos = Array.from(ingresosPorCliente.values()).reduce((acc, c) => acc + c.ingresosCentimos, 0);
  const clientesDistintosEnRango = ingresosPorCliente.size;
  const valorMedioPorClienteCentimos =
    clientesDistintosEnRango > 0 ? Math.round(ingresosTotalRangoCentimos / clientesDistintosEnRango) : 0;
  ...
  for (const [clienteId, info] of porCliente.entries()) primeraVisitaPorCliente.set(clienteId, info.primera);

  const serieMap = new Map<string, { nuevos: Set<string>; recurrentes: Set<string> }>();
  for (const c of citasRango) {
    const { clave } = bucketDe(c.inicio, granularidad);
    const actual = serieMap.get(clave) ?? { nuevos: new Set<string>(), recurrentes: new Set<string>() };
  ...
  const serieNuevosRecurrentes = buckets.map((b) => {
    ...
    return { etiqueta: b.etiqueta, nuevos: datos?.nuevos.size ?? 0, recurrentes: datos?.recurrentes.size ?? 0 };
  });
