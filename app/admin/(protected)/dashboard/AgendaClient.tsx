...
export default function AgendaClient({
  ...
  const [profesionalesDia, setProfesionalesDia] = useState<
  ...
  async function cargarCitas() {
    ...
    const res = await fetch(
      `/api/admin/citas?sedeId=${sedeId}&fecha=${fechaInicio}&fechaFin=${fechaFinRango}`,
    );
  ...
  const leyendaServicios = useMemo(() => {
    ...
    for (const c of citas) {
      const s = c.servicio;
      if (s?.color && !vistos.has(s.id))
        vistos.set(s.id, { nombre: s.nombre, color: s.color });
    }
    if (vistos.size === 0) {
  ...
  async function avisarDisponible(id: string) {
    if (
      !confirm(
        "¿Avisar por WhatsApp a tu siguiente cliente de que ya estás disponible?",
      )
    )
      return;
    setAvisando(id);
    const res = await fetch(`/api/admin/citas/${id}/avisar-disponible`, {
      method: "POST",
    });
    const json = await res.json();
    setAvisando(null);
            ...
            {vista === "dia" ? (
              ...
              <div className="flex items-center gap-2 text-sm text-stone-700">
                ...
                <span className="whitespace-nowrap">
                  {diasSemana.length > 0 &&
                    `${formatoFechaCorta(diasSemana[0])} – ${formatoFechaCorta(diasSemana[6])}`}
                </span>
                <button
                  onClick={() => moverFecha(7)}
                  className="rounded-lg border border-stone-300 px-2 py-1 hover:border-stone-400"
      ...
      {dias.map((dia) => (
        <div
          ...
          className={
            ...
            (dia === hoy ? "border-brand-yellow" : "border-stone-200")
          }
        >
          ...
          <div className="space-y-2">
            ...
            {(citasPorDia.get(dia) ?? []).map((cita) => (
              <div
                ...
                style={
                  cita.servicio?.color && cita.estado !== "cancelada"
                    ? { borderLeftColor: cita.servicio.color }
                    : undefined
                }
              >
                <div className="flex items-center gap-1 font-medium text-stone-900">
                  ...
                  {cita.estado === "completada" && (
                    <span className="text-emerald-600" title="Completada">
                      ✓
                    </span>
                  )}
                  ...
                  <div className="flex flex-wrap justify-end gap-1">
                    ...
                    {cita.estado === "confirmada" && (
                      <>
                        ...
                        <button
                          onClick={() =>
                            onCambiarEstado(cita.id, "no_presentada")
                          }
...
function NuevaCitaForm({
  ...
  useEffect(() => {
    ...
    fetch(
      `/api/profesionales?sedeId=${sedeId}&servicioId=${servicioId}&fecha=${fecha}`,
    )
      .then((r) => r.json())
  ...
  const horasUnicas = Array.from(
    new Map(slots.map((s) => [s.hora_inicio, s])).values(),
  );
    ...
    <div className="space-y-3 rounded-lg border border-stone-200 bg-white p-4">
      ...
      <div className="flex flex-wrap gap-2">
        ...
        {horasUnicas.length === 0 && (
          <p className="text-sm text-stone-400">No hay huecos ese día.</p>
        )}
      </div>
